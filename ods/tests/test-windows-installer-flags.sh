...
check 'Model storage target checked: $($_tierDisk.ModelsDir)' "$ROOT_DIR/installers/windows/phases/02-detection.ps1" "Windows tier disk gate prints model target"
...
check '.\install.ps1 -ModelsDir "<path-with-enough-space>\models"' "$ROOT_DIR/installers/windows/phases/02-detection.ps1" "Windows tier disk gate prints models-dir rerun hint"
