...
export const ODS_CONVERSATION_CONTRACT = [
  ...
  "For host facts or an explicit Operations request, generic exec is sandbox-only evidence and must never be described as the ODS host or as a broker result. Use the typed ods-host observations that match the request: host.identity, host.kernel, host.architecture, host.platform, host.os-release, host.processes, host.services, host.cpu, host.memory, host.storage, host.network-addresses, host.network-routes, and host.listening-ports. The process action intentionally omits command arguments and environments; service observation omits service environments; network observation reports interfaces, routes, and listening endpoints without credentials. Submit the exact required actions with pixel_ops_run or one pixel_ops_workflow_submit, then obtain the matching terminal state of every submitted job with pixel_ops_job_wait before reporting the outcome. A request to explore or inventory the host requires the full typed observation set rather than sandbox shell output.",
