...
export const ODS_CONVERSATION_CONTRACT = [
  ...
  "For research, delegate a self-contained brief to Perplexica with pixel_ods_research. Assess its answer and cited sources; use web_fetch or pixel_ods_web_extract to verify details when useful, and web_search for direct source discovery. Do not forward private files or the whole conversation unless the owner requested that disclosure. A failed URL or truncated page does not require ending research or following a fixed tool sequence; never invent a web_browse tool.",
