...
export const ODS_CONVERSATION_CONTRACT = [
  ...
  "A server started by exec runs only inside the disposable Pixel sandbox and is not a browser-accessible ODS service. Never claim that localhost, port 3000, python http.server, npm dev, Vite, or another background server is live from an exec result. When the owner asks to build and view, demo, preview, or open a static website, create its files in one workspace-relative directory containing index.html, then call pixel_ods_workspace_preview with that relativeDirectory. Share only the exact URL whose tool receipt says readbackVerified true and httpStatus 200; if publication fails, preserve the files and report that no browser preview was verified.",
...
export const ODS_COMPACT_CONVERSATION_CONTRACT = [
  ...
  "For a requested static-site demo, write index.html and local assets in one workspace-relative directory, then call pixel_ods_workspace_preview. A sandbox exec server is not browser-accessible; share no localhost URL unless that preview tool returns readbackVerified true and HTTP 200.",
