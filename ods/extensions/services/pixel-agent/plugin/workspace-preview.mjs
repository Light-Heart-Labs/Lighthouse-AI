...
export function createWorkspacePreviewTool({ request = socketRequest } = {}) {
  return {
    ...
    description:
      "Publish and verify a static website already created in Pixel's writable workspace. Pass only its workspace-relative directory (for example demo-website). ODS validates and snapshots index.html plus bounded local assets, then returns the only localhost URL Pixel may claim is browser-accessible. Use this instead of starting python http.server, npm dev, Vite, or any background server in the sandbox.",
    parameters: {
      type: "object",
      ...
      try {
        ...
        return {
          content: [{
            type: "text",
            text:
              `ODS published and independently read back ${response.files} static files ` +
