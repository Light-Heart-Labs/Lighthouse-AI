...
def _select_configured_services(candidates: list[str]) -> list[str]:
    """Narrow the built-in LLM alternatives to the configured runtime route."""
...
def calculate_feature_status(feature: dict, services: list, gpu_info: Optional[GPUInfo]) -> dict:
    ...
    services_any_ok = (not selected_services_any) or any(
        svc in services_available for svc in selected_services_any
    )
    services_ok = services_all_ok and services_any_ok
    services_all_missing = [
        svc for svc in required_services if svc not in services_available
    ]
    services_any_missing = [
        svc for svc in selected_services_any if svc not in services_available
    ]
    services_missing = services_all_missing + (
        [] if services_any_ok else services_any_missing
    )
    ...
    enabled_any_ok = (not selected_enabled_any) or any(
        ...
        for svc in selected_enabled_any
    )
...
async def api_features(api_key: str = Depends(verify_api_key)):
    ...
    for f in feature_statuses:
        if f["status"] == "available":
            ...
            selected_any = requirements["servicesAnySelected"]
            if selected_any and requirements["servicesAnyMissing"] == selected_any:
                label = (
                    f"one of {' or '.join(selected_any)}"
                    if len(selected_any) > 1
                    else selected_any[0]
                )
